...
def plot_scores(scores, details, x_values, x_label, title=None, figsize=(16, 9)):
    """plot_scores plotting given values for the results of learning
    :param scores, details, x_values, y_values, title and figsize"""
    marks = ['r-', 'b-', 'y-', 'g-']
...
def summarize_model(model, train_x, train_y, test_x, test_y):
    """Summaraizing the model, calculating errors between train predictions and real results both for train and test data"""
...
def run_logistic_regression(X_train, y_train, X_test, y_test, alpha, num_of_features):
    """Running logistic regression on model
    :param alpha: """
    zero_one_y_train = [0 if c == 'M' else 1 for c in y_train] # M =0, B=1
    zero_one_y_test = [0 if c == 'M' else 1 for c in y_test] # M =0, B=1
...
def run_stochastic_logistic_regression(X_train, y_train, X_test, y_test, alpha, num_of_features):
    """Running stochastic logistic regression on model
    :param alpha: 
...
def runAlgorithmComparison():
    ...
    X, Y = parseExcelDataSet("real_project_data.xls")
    ...
    for ker in svm_kernels:
        ker_score = []
        for c in C_arr:
            clf = SVC(kernel=ker, C=c, degree=2) #different c penalty, degree =2 polynomial kernel function
    ...
    for solver in solvers:
        nn_score = []
        for a in alpha_arr:
            clf = MLPClassifier.MLPClassifier(alpha=a, solver=solver)
            clf.fit(X_train, y_train)
            score = clf.score(X_test, y_test)
...
def runKFoldAlgorithm():
    ...
    svm_kernels = ["linear", "rbf", "sigmoid"] #list of kernels
    kernels_score = []
    for ker in svm_kernels: # for several kernels
        ker_score = []
        for c in C_arr:
            clf = SVC(kernel=ker, C=c, degree=2) #activating SVM algorithm with different c - error penalty
            train_score, test_score = run_k_fold(X, Y, clf, 'SVM', draw_scores=True) #run k_fold with plotting
...
def calculateResults(argv, clf, algorithm):
    X_train, y_train = parseExcelDataSet("real_project_data.xls")
...
def main(argv):
    ...
    while True:
        print("\n--------- Welcome to Python machine learning final project ---------")
        print("""
        Menu:
